...
window.addEventListener('DOMContentLoaded', (e)=>{
    ...
    //console.log(grpId)
    axios.get(`http://43.204.110.172:3000/isAdmin?grpId=${grpId}`, { headers: {"Authorization" : token}})
    .then(user=>{
       ...
       // console.log(user.data.user.isAdmin)
        ...
        // setInterval(()=>{
            axios.get(`http://43.204.110.172:3000/getMessages?grpId=${grpId}`, { headers: {"Authorization" : token} })
            .then(res=>{
                //console.log(res)
...
sendBtn.addEventListener('click', (e)=>{
   e.preventDefault()
    ...
    if(message.value == ''){
        ...
        axios.post('http://43.204.110.172:3000/addMessage', {groupid: grpId, msg: message.value}, { headers: {"Authorization" : token}})
        .then(res=>{
